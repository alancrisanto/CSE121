...
btnSubmit = document.querySelector("#submitTask");

btnSubmit.addEventListener("click", newTask);
...
function renderTasks(tasks) {
	...
	listElement.innerHTML = listTask.join("");
}
...
function newTask() {
	...
	tasks.push({ detail: inputValue.value, completed: false });
	// render out the list.
	console.log(tasks);
	renderTasks(tasks);
	inputValue.value = "";
...
function manageTasks(event) {
	...
	const closestParent = event.target.closest("li");
	...
	if (event.target.dataset.function === "delete") {
		removeTask(closestParent);
	}
	if (event.target.dataset.function === "completed") {
		completeTask(closestParent);
	}
}
